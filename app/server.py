...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        log.exception("failed to load Higgs Audio engine")
        raise

    app.state.engine = engine
    log.info(
        "engine ready: model=%s device=%s dtype=%s sample_rate=%d",
        settings.higgs_model,
        engine.device,
        engine.dtype_str,
        engine.sample_rate,
    )
...
app = FastAPI(title="higgs-audio-open-tts", version="1.0.0", lifespan=lifespan)
...
async def clone(
    ...
    tmp_dir = Path(tempfile.gettempdir()) / "higgs-open-tts"
