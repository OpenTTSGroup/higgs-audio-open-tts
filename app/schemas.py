...
class SpeechRequest(BaseModel):
    ...
    voice: str = Field(
        description='"file://<id>" for a disk reference voice.'
    )
    ...
    instructions: Optional[str] = Field(
        default=None,
        description="Higgs inline control tokens (e.g. '<|emotion:amusement|>') prepended to input.",
    )
    ...
    max_new_tokens: Optional[int] = Field(
        ...
        ge=1,
        le=16384,
        description="Maximum generated multi-codebook steps.",
    )
...
class DesignRequest(BaseModel):
    ...
    instruct: Optional[str] = Field(
        default=None,
        description="Higgs inline control tokens or style hints prepended to input. "
        "Null or empty string uses the engine default voice.",
    )
    response_format: Optional[ResponseFormat] = Field(
        default=None,
        description="Output container/codec; defaults to the service setting.",
