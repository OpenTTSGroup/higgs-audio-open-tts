...
| `HIGGS_MEM_FRACTION_STATIC` | （自动） | GPU 显存中 KV cache 占比（`0.0`–`1.0`）。遇到 "thinker KV cache" 报错时调高。 |
| `HIGGS_BACKEND_URL` | （空） | 外部 sglang-omni 后端 URL；设置后引擎连接该地址而非自行启动 |
| `HIGGS_TEMPERATURE` | `0.8` | 默认采样温度 |
| `HIGGS_TOP_K` | `50` | 默认 top-k 采样 |
| `HIGGS_MAX_NEW_TOKENS` | `1024` | 默认最大生成 token 数 |
...
| `max_new_tokens` | int | `1024` | extension | 最大生成步数 `[1, 16384]`。 |
...
| `max_new_tokens` | int | `1024` | extension | 最大生成步数。 |
| `seed` | int | `null` | extension | 随机种子。 |
